...
import api from "../utils/axiosInstance"; // ✅ GANTI dari axios biasa
...
const css = `
  ...
  @media (max-width: 480px) { .stat-value { font-size: 26px; } .stat-card { padding: 16px 14px 14px; } }
  .stat-value.loading {
  ...
  @media (max-width: 480px) { .panel { padding: 16px; } }
  .panel-header {
    display: flex;
  ...
  .panel-title { font-size: 13px; font-weight: 500; color: #111; letter-spacing: 0.01em; }
  .panel-subtitle { font-size: 11.5px; color: #bbb; }
  .panel-link { font-size: 11.5px; color: #999; cursor: pointer; transition: color 0.15s; }
  ...
  .dash-sections { display: flex; flex-direction: column; gap: 10px; margin-top: 10px; }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // ✅ path relatif → lewat Vite proxy saat dev, langsung ke server saat production
        ...
        console.error("[Dashboard] Gagal fetch:", err.message);
        setStats([
